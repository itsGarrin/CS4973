...
def main():
    ...
    st.markdown(
        "<h1 style='text-align: center; margin-bottom: 3rem;'>✈️ Travel Agent LLM</h1>",
    ...
    st.markdown(
        ...
        .message { border-radius: 10px; margin: 10px 0; padding: 10px 15px; color: #333; }
        .user-message { background-color: #e6f7ff; box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.1); }
        .agent-message { background-color: #ffe6e6; box-shadow: 0px 0px 5px rgba(0, 0, 0, 0.1); }
        </style>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.sidebar:
        ...
        origin = st.text_input(
            "Origin (Airport Code)", max_chars=3, placeholder="e.g., ATL"
        )
        destination = st.text_input(
            "Destination (Airport Code)", max_chars=3, placeholder="e.g., SEA"
        )
        travel_date = st.date_input(
            "Travel Date", min_value=date(2023, 1, 1), max_value=date(2023, 12, 31)
        )
        find_btn = st.button("Find Flights")

        st.header("Book a Flight")
        flight_id = st.number_input("Flight ID to Book", value=1, min_value=1)
        book_btn = st.button("Book Flight")
    ...
    with col1:
        if find_btn:
            ...
            logging.info(
                f"Find Flights button clicked with Origin: {origin}, Destination: {destination}, Date: {travel_date}"
            )
                    ...
                    if isinstance(response, FindFlightsResponse):
                        if response.available_flights:
                            for flight_id in response.available_flights:
                                ...
                                st.success(
                                    f"✈️ **Flight ID**: {flight.id}, **Airline**: {flight.airline}, **Flight Number**: {flight.flight_number}, **Origin**: {flight.origin}, **Destination**: {flight.destination}, **Departure**: {flight.departure_time}, **Arrival**: {flight.arrival_time}, **Seats Available**: {flight.available_seats}"
                                )
                        else:
        ...
        if book_btn:
            ...
            if flight_id > 0:
                ...
                if isinstance(response, BookFlightResponse):
                    ...
                    if booked_flight:
                        st.success(
                            f"Flight with ID {booked_flight} booked successfully!"
                        )
                        logging.info(
                            f"Flight with ID {booked_flight} booked successfully."
                        )
                        ...
                        st.error(
                            "Failed to book the flight. No available seats or invalid flight ID."
                        )
                        logging.error(
                            "Booking failed - no available seats or invalid flight ID."
                        )
                else:
                    st.error(
                        response.text
                        or "An unexpected error occurred during booking. Please try again."
                    )
                    logging.error(
                        f"Unexpected response during booking: {response.text}"
                    )
            else:
